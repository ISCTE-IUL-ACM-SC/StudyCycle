...
fn test_errors_used() {
  ...
  current_dir.pop();
  current_dir.pop();
  for error in StudyCycleErrorType::iter() {
    let search = format!("StudyCycleErrorType::{error}");
    ...
    if grep_all_out.is_empty() {
      println!("StudyCycleErrorType::{} is unused", error);
      unused_error_found = true;
    }
    if search != "StudyCycleErrorType::UntranslatedError" && grep_all_out == grep_apub_out {
      println!("StudyCycleErrorType::{} is only used for federation", error);
      unused_error_found = true;
    }
  }
