use crate::error::{StudyCycleErrorExt, StudyCycleErrorType, StudyCycleResult};
use regex::Regex;
...
pub fn check_slurs(text: &str, slur_regex: &Regex) -> StudyCycleResult<()> {
  if let Err(slurs) = slur_check(text, slur_regex) {
    Err(anyhow::anyhow!("{}", slurs_vec_to_str(&slurs))).with_studycycle_type(StudyCycleErrorType::Slurs)
  } else {
    Ok(())
  }
}

pub fn check_slurs_opt(text: &Option<String>, slur_regex: &Regex) -> StudyCycleResult<()> {
  match text {
...
mod test {
  ...
  #[test]
  fn test_slur_filter() -> StudyCycleResult<()> {
